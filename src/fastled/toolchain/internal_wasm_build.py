...
def configure_project_root(project_root: Path) -> None:
    """Bind the copied build logic to a specific FastLED checkout."""
    global PROJECT_ROOT
    global CROSS_FILE
    global WASM_PCH_HEADER
    global BUILD_DIR_PREFIX
    global BUILD_FLAGS_TOML
    global _cached_fingerprint
    global _cached_fingerprint_time
...
def get_sketch_compile_flags(mode: str = "quick") -> list[str]:
    ...
    return defines + compiler_flags


def get_link_flags(mode: str = "quick") -> list[str]:
    """Read link flags from FastLED's build_flags.toml."""
    config = _load_toml()
    build_mode_config = config.get("build_modes", {}).get(mode, {})
...
def _build_env() -> dict[str, str]:
    """Create an emscripten-ready environment using fastled-wasm's toolchain setup."""
...
def get_emcc() -> str:
    """Resolve the emcc entrypoint from fastled-wasm's installed toolchain."""
    from fastled.toolchain.emscripten import EmscriptenToolchain

    return str(EmscriptenToolchain()._find_compilers().emcc)


def get_emar() -> str:
    """Resolve the emar entrypoint from fastled-wasm's installed toolchain."""
    from fastled.toolchain.emscripten import EmscriptenToolchain

    return str(EmscriptenToolchain()._find_compilers().emar)


def get_native_emcc() -> str | None:
    """Use FastLED's compiled native emcc launcher when available."""
    suffix = ".exe" if sys.platform == "win32" else ""
    candidate = PROJECT_ROOT / ".build" / "native-tools" / f"ctc-emcc{suffix}"
    if candidate.exists():
        return str(candidate)
    return None


def run_emcc(args: list[str], cwd: str | None = None) -> int:
    """Run emcc via subprocess using fastled-wasm's configured environment."""
    result = subprocess.run(_tool_command(get_emcc()) + args, cwd=cwd, env=_build_env())
    return result.returncode


def _tool_command(path: str) -> list[str]:
    tool_path = Path(path)
...
def _get_emcc_version_signature() -> str:
    ...
    global _cached_emcc_version_key, _cached_emcc_version_value, _cached_emcc_version_time
    emcc = get_emcc()
    now = time.monotonic()
    if (
...
def _recover_stale_wasm_build(build_dir: Path) -> bool:
    ...
    try:
        ...
        handle_keyboard_interrupt(ki)
        return False  # unreachable on main thread, satisfies pyright
    except Exception as e:
...
def build_sketch_pch(
    ...
    if rc != 0:
        ...
        if library_archive.exists():
            emar = get_emar()
            subprocess.run(
                _tool_command(emar) + ["r", str(library_archive), str(pch_codegen_o)],
                cwd=str(PROJECT_ROOT),
                env=_build_env(),
                check=False,
            )
...
def link_wasm(
    ...
    # Disabled when SEPARATE_DWARF_URL is used — emcc handles DWARF
...
def _run_npm(args: list[str], cwd: Path) -> int:
    ...
    if npm is None:
        return 1
    return subprocess.run([npm, *args], cwd=str(cwd)).returncode
...
def _run_npx(args: list[str], cwd: Path) -> int:
    ...
    if npx is None:
        return 1
    return subprocess.run([npx, *args], cwd=str(cwd)).returncode
...
def copy_templates(output_dir: Path) -> None:
    """Copy template files using fastled-wasm's bundled esbuild frontend pipeline."""
...
def build(
    ...
    try:
        ...
        handle_keyboard_interrupt(ki)
        return 1  # unreachable on main thread, satisfies pyright
    except Exception as e:
