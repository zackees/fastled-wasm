...
class CLINoPlatformIOTest(unittest.TestCase):
    ...
    def test_no_platformio_flag_recognized(self) -> None:
        ...
        self.assertIn("--no-platformio", result.stdout, "--no-platformio should appear in help")
        self.assertIn("Bypass PlatformIO constraints", result.stdout, "Help text should be present")
    ...
    def test_no_platformio_flag_parsing(self) -> None:
        ...
        self.assertIn("--no-platformio", result.stdout, "--no-platformio should be in help output")
    ...
    def test_no_platformio_flag_forces_local_mode(self) -> None:
        ...
        try:
            result = subprocess.run(
                ["uv", "run", "fastled", str(TEST_SKETCH_DIR), "--no-platformio", "--just-compile"],
                cwd=WORKSPACE_ROOT,
                capture_output=True,
                text=True,
            ...
            self.assertTrue(
                "localhost" in output.lower() or "local" in output.lower() or "docker" in output.lower(),
                "Should attempt local compilation mode"
            )
            
            # If compilation succeeded, verify success
            if result.returncode == 0:
                self.assertIn("compilation success", output.lower(), "Should indicate compilation success")
                ...
                self.assertNotIn("unrecognized arguments", output, "Flag should be recognized")
                print(f"ℹ️  --no-platformio flag processed correctly (exit code: {result.returncode})")
                print(f"Output: {output[:500]}...")  # Print first 500 chars for debugging
                
        except subprocess.TimeoutExpired:
            self.fail("Command timed out - this suggests the flag was processed but compilation took too long")
    ...
    def test_no_platformio_cli_argument_structure(self) -> None:
        ...
        for flags in conflicting_flags:
            with self.subTest(flags=flags):
                ...
                self.assertEqual(result.returncode, 0, f"Should accept --no-platformio with {flags}")
                self.assertNotIn("error:", result.stderr.lower(), f"No errors with --no-platformio + {flags}")
        ...
        for sketch_dir in test_sketches:
            if sketch_dir.exists():
                with self.subTest(sketch_dir=sketch_dir):
                    ...
                    result = subprocess.run(
                        ["uv", "run", "fastled", str(sketch_dir), "--no-platformio", "--help"],
                        cwd=WORKSPACE_ROOT,
                        capture_output=True,
                        text=True,
                        timeout=30
                    ...
                    self.assertEqual(result.returncode, 0, f"Should work with sketch at {sketch_dir}")
                    self.assertIn("--no-platformio", result.stdout, "Flag should be present in help")
