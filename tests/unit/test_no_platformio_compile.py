...
class NoPlatformIOCompileTester(unittest.TestCase):
    ...
    @unittest.skipUnless(_enabled() and _docker_available(), "Requires Docker for no-platformio compilation.")
    def test_no_platformio_compile_success(self) -> None:
        ...
        with Api.server() as server:
            ...
            self.assertTrue(result.success, f"No-platformio compilation failed. Output: {result.stdout}")
            
            # Verify we got actual compiled output
            self.assertTrue(len(result.zip_bytes) > 0, "No compiled output received from no-platformio mode")
            
            # Verify stdout contains expected compilation messages
            self.assertIsNotNone(result.stdout, "No stdout received from no-platformio compilation")
            
            # Print no-platformio compilation info for debugging
            print(f"No-platformio compilation successful!")
            ...
            if result.zip_bytes:
                print("Successfully received compiled WASM output from no-platformio mode")
    ...
    def test_no_platformio_compile_with_project_init(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            with Api.server() as server:
                ...
                # Verify no-platformio compilation succeeded
                self.assertTrue(result.success, f"No-platformio compilation of initialized project failed. Output: {result.stdout}")
                self.assertTrue(len(result.zip_bytes) > 0, "No compiled output received from no-platformio initialized project")
                
                print(f"Successfully compiled initialized Blink project in no-platformio mode!")
    ...
    def test_no_platformio_api_structure_and_workflow(self) -> None:
        ...
        if not docker_available:
            print("Docker not available - demonstrating no-platformio API structure without compilation")
        ...
        self.assertTrue(hasattr(Api, 'server'), "Api should have server method for no-platformio mode")
        self.assertTrue(hasattr(Api, 'project_init'), "Api should have project_init method")
        
        # Test BuildMode enum
        self.assertTrue(hasattr(BuildMode, 'QUICK'), "BuildMode should have QUICK for no-platformio")
        self.assertTrue(hasattr(BuildMode, 'DEBUG'), "BuildMode should have DEBUG for no-platformio")
        self.assertTrue(hasattr(BuildMode, 'RELEASE'), "BuildMode should have RELEASE for no-platformio")
