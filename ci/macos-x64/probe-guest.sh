...
# Safari. Recovery has no `open`, a command-line argument is read as a
# sandboxed file path (Safari then asks for consent nothing here can grant), and
# LaunchServices brings Safari forward without loading the URL. A home page
# preference, written before Safari first runs so no cached preferences override
# it, makes a plain launch open its window on the page.
...
<key>HomePage</key><string>$URL/?who=safari</string>
<key>NewWindowBehavior</key><integer>0</integer>
<key>NewTabBehavior</key><integer>0</integer>
</dict></plist>
PLIST
done
/Applications/Safari.app/Contents/MacOS/Safari > $R/safari.log 2>&1 &
sleep 45
